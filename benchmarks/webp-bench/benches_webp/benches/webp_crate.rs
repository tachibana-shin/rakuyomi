...
fn decode_rgb_webp(data: &[u8]) -> Vec<u8> {
    let img = Decoder::new(data).decode().unwrap();
    let w = img.width() as usize;
    let h = img.height() as usize;
    match img.layout() {
        PixelLayout::Rgb => img.to_vec(),
        PixelLayout::Rgba => {
            let mut rgb = Vec::with_capacity(w * h * 3);
            for rgba in img.chunks_exact(4) {
                rgb.push(rgba[0]);
                rgb.push(rgba[1]);
                rgb.push(rgba[2]);
            }
...
macro_rules! bench_group {
    ($c:ident, $name:expr, $data:expr) => {
        ...
        g.sample_size(30);
        g.bench_function("webp", |b| b.iter(|| black_box(decode_rgb_webp(black_box($data)))));
        g.finish();
    };
}
