...
impl Database {
    ...
    pub async fn hot_replace(&self, buf: &[u8]) -> Result<()> {
        ...
        // Run migrations on the new pool, with the same policy as startup
    ...
    /// Returns the chapter IDs of every chapter belonging to a manga currently
    /// in the user's library. Used to compute per-manga downloaded storage usage.
    pub async fn get_library_chapter_ids(&self) -> Result<Vec<ChapterId>> {
        use sqlx::Row;

        let rows = sqlx::query(
            r#"
                SELECT ci.source_id, ci.manga_id, ci.chapter_id
                FROM chapter_informations ci
                JOIN manga_library ml
    ...
    /// Returns the chapter IDs of every cached chapter belonging to a single manga.
            ...
            crate::settings::LibrarySortingMode::UnreadAsc => {
                sqlx::query_as!(
                    ...
                    LEFT JOIN chapter_informations ci
                        ...
                        AND ci.chapter_number > COALESCE(lr.last_read_chapter, -1)
                    GROUP BY ml.source_id, ml.manga_id, lti.last_read_time
                    ORDER BY COUNT(ci.chapter_number) ASC
                    "#
                )
                .fetch_all(&*self.pool.read().await)
                .await?
            ...
            crate::settings::LibrarySortingMode::UnreadDesc => {
                sqlx::query_as!(
                    ...
                    LEFT JOIN chapter_informations ci
                        ...
                        AND ci.chapter_number > COALESCE(lr.last_read_chapter, -1)
                    GROUP BY ml.source_id, ml.manga_id, lti.last_read_time
                    ORDER BY COUNT(ci.chapter_number) DESC
                    "#
                )
                .fetch_all(&*self.pool.read().await)
                .await?
