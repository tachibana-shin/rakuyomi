...
to KOReader (Rakuyomi) devices.

## Architecture

- **Bot** — grammY, handles `/link`, `/devices`, cookie ingestion via text
  messages
- **API** — Hono + `@hono/zod-openapi`, serves cookie data to the Rust backend
- **DB** — Turso (libSQL), stores device registrations and per-device cookie
  JSON blobs
